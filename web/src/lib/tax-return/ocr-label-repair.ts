...
const WORD_REPAIRS: Array<[RegExp, string]> = [
  ...
  [/\bcontinuing\s+eduation\b/gi, "continuing education"],
  [/\bsection\s+a\s+w\s+wages?\b/gi, "section a wages"],
  [/\brepairs?\s+and\s+maint(?:enance)?\s+a\b/gi, "repairs and maintenance"],
  [/\brents?\s*\|/gi, "rent"],
